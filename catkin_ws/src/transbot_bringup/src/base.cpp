...
RobotBase::RobotBase() :
    ...
    nh_private.param<float>("odom_publish_rate", odom_publish_rate_, 20.0);
    nh_private.getParam("is_namespace", namespace_);
    nh_private.param<bool>("is_multi_robot", is_multi_robot_,false);
    str1 = namespace_;
    last_publish_time_ = ros::Time::now();
    odom_timer_ = nh_.createTimer(ros::Duration(1.0 / odom_publish_rate_), &RobotBase::publishOdometry, this);
}
...
void RobotBase::publishOdometry(const ros::TimerEvent& event) {
    ros::Time current_time = event.current_real;
    if (last_publish_time_.toSec() == 0) {
        last_publish_time_ = current_time;
    }

    vel_dt_ = (current_time - last_publish_time_).toSec();
    last_publish_time_ = current_time;
    if (vel_dt_ < 0) {
        vel_dt_ = 0;
    }
